<template>
  <div>
    <v-app-bar id="home-app-bar" app color="#fff9ec" height="80">
      <router-link to="/">
        <base-img :src="require('@/assets/logo_small.png')" contain max-width="80" />
      </router-link>
      <v-spacer />
      <div>
        <v-row class="hidden-sm-and-down" optional>
          <v-col>
            <button v-if="(this.isLogged)" class="btn-6" @click="Logout()"><span>Logout</span>
            </button></v-col>
            <v-col v-if="(!this.isLogged)" min-width="96" text>
          ...
          <v-snackbar v-model="snackbar" :timeout="timeout" color="cyan darken-2">
            {{ text }}
          </v-snackbar>
        </div>
